    ...
    for sequence, seq_len in zip(sequences, sequence_lengths):
        ...
        if (len(df_diamond) == 0):
            ...
            for id in uniprot_ids:
                ...
                df_elm["target_start"] = int(df_diamond["target_start"])
                df_elm["target_end"] = int(df_diamond["target_end"])
...
def elm(sequence, uniprot=False, json=False, input_file=None, reference=ELM_INSTANCES_FASTA, out=None, sensitivity= "very-sensitive", verbose=True):
    ...
    if len(df) == 0:
        ...
        if not uniprot and len(df) > 0:
            try:
                target_start_values = int(df['target_start'].values)
                target_end_values = int(df['target_end'].values)
