...
def gpt(
    ...
    """
    [WARNING OpenAI API calls are only 'free' for the first three months after generating your OpenAI Account
    ...
    Args:
        ...
        max_tokens (int, optional): The maximum number of tokens (words or subwords) in the generated text (defaults to 1024).
    ...
    Example:
        ...
        >>> response = generate_response(prompt, api_key, engine="curie", max_tokens=64, n=3, stop="\n", temperature=0.7)
        >>> print(response)
    ...
    logging.info(f"Total tokens used for API call to engine '{engine}': {response['usage']['total_tokens']}")

    texts = " ".join(response.choices[0]["text"].strip().replace("\n","").split(" ")[1:])
