...
import os
import subprocess
from typing import Literal, Callable, Any, TypeAlias, TypeVar
...
from .constants import CBIO_CANCER_TYPE_TO_TISSUE_DICTIONARY, CBIO_CANCER_TYPE_ACRONYM_TO_TISSUE_DICTIONARY
...
def download_cbioportal_data(study_ids: list[str], verbose: bool = False, out_dir: str | None = None,
                             confirm_download: bool = False) -> bool:
    """
    ...
    for study_id in study_ids:
        ...
        for file_type in file_types:
            try:
                ...
                assert fields[
                           "version"] == "https://git-lfs.github.com/spec/v1", f"Cannot handle git-lfs version {fields['version']}"
    ...
    if plan and plan.objects:
        do_download = input(
            f"Do you want to download {_describe_bytes(plan.total_size)} to {actual_out_dir}? (y/n) ").lower().strip() == "y"
        if do_download:
...
def find_study_ids_by_keywords(key_words: list[str] | set[str]) -> list[str]:
    ...
    cancer_type_acronym_dict = {_extract_study_name(individual_study['name']): individual_study['cancerTypeId'] for
                                individual_study in studies}
...
class _GeneAnalysis:
    def __init__(
        ...
        assert self.merge_type in [_SYMBOL, _ENSEMBL], "merge_type must be either 'Symbol' or 'Ensembl'"
    ...
    def _create_single_study_dataframe(self, study_id: str) -> pd.DataFrame:
        ...
        if self.merge_type == _ENSEMBL:
            if 'Gene' in mutation_df.columns and mutation_df['Gene'].str.startswith('ENSG').any():
                mutation_df.rename(columns={'Gene': 'Ensembl_Gene_ID'}, inplace=True)
                if self.remove_non_ensembl_genes:
                    mutation_df = mutation_df[mutation_df['Ensemble_Gene_ID'].str.startswith('ENSG')]
                ...
                mutation_df.progress_apply(_get_valid_ensembl_gene_id, axis=1, transcript_column="Transcript_ID",
        ...
        occurrences_df = aggregated_df.groupby([self.column_for_merging, 'Tumor_Sample_Barcode']).size().reset_index(
            name='mutation_occurrences')
        ...
        if os.path.exists(os.path.join(data_folder, "cna.txt")):
            ...
            melted_cna = self.df_collection[study_id]['cna_binary'].melt(id_vars=['Hugo_Symbol'],
                                                                         var_name='Tumor_Sample_Barcode',
                                                                         value_name='cna_occurrences')

            final_df = pd.merge(final_df, melted_cna[['Hugo_Symbol', 'Tumor_Sample_Barcode', 'cna_occurrences']],
                                on=['Hugo_Symbol', 'Tumor_Sample_Barcode'], how='outer')
        ...
        if os.path.exists(os.path.join(data_folder, "sv.txt")):
            ...
            melted_sv = pd.melt(self.df_collection[study_id]['sv'], id_vars=['Sample_Id'],
            ...
            final_df = pd.merge(final_df, sv_occurrences[['Hugo_Symbol', 'Tumor_Sample_Barcode', 'sv_occurrences']],
                                on=['Hugo_Symbol', 'Tumor_Sample_Barcode'], how='outer')
        ...
        final_df.rename(columns={'Cancer Type': 'cancer_type', 'Cancer Type Detailed': 'cancer_type_detailed'},
                        inplace=True)

        final_df['tissue'] = final_df['cancer_type'].map(CBIO_CANCER_TYPE_TO_TISSUE_DICTIONARY).fillna('unclassified')
    ...
    def _create_study_dataframes(self) -> pd.DataFrame:
        ...
        for study_id in self.study_ids:
            ...
            with open(f"{self.data_dir}/{study_id}/mutations.txt", 'r') as file:
                lines = file.readlines()[1:]  # cut out a comment (needed at least for msk_impact_2017)
        ...
        if variation_type != "cna_nonbinary":
            ...
            if stratification in simple_merge_by_stratification:
                ...
                unique_samples_info = final_df[
                    ['Tumor_Sample_Barcode', 'cancer_type', 'cancer_type_detailed', 'study_id',
                     'tissue']].drop_duplicates()

                hugo_mask = final_df['Hugo_Symbol'].isin([gene for gene in gene_list if not gene.startswith('ENSG')])
                ...
                unexpressed_genes = [gene for gene in gene_list if gene not in existing_genes]
                ...
                if filter_category is None or stratification != "sample":  # no filtering
                    df_for_heatmap_very_final: pd.DataFrame = merged_df.groupby([self.column_for_merging, stratification])[variation_type].sum().reset_index()
                else:
            ...
            if self.column_for_merging == "Ensembl_Gene_ID":
                ...
                raise NotImplementedError("Ensembl to Hugo conversion not implemented yet")
