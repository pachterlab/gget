...
def tget(searchwords, species, limit=None):
    """
    Function to query Ensembl for transcripts based on species and free form search terms. 
    
    The variable "searchwords" is a list strings containing the free form search terms 
    (e.g.searchwords = ["GABA", "gamma-aminobutyric acid"]).
    The search is not case-sensitive.
    
    "Limit" limits the number of search results to the top {limit} transcripts found.
    ...
    for searchword in searchwords:
        ...
        if limit != None:
            cursor.execute(
                f"select * from transcript where description like '%{searchword}%' limit {limit};"
            )
        # Else, fetch all transcripts for which the searchword appears in the description
        else:
            cursor.execute(f"select * from transcript where description like '%{searchword}%';")
        # Fetch the search results and save as "res"
        res = cursor.fetchall()
